...
from agents.bedrock_agent import run_agent_bedrock

# --- Streamlit Config ---
st.set_page_config(page_title="Agentic Job Apply — Minimal UI", layout="centered")
st.title("Quick-Apply — Agentic AI Job Submitter")
st.markdown(
    "Upload your resume, pick a date, and press Submit. The agent will generate ATS-optimized emails & resumes using **Amazon Titan via AWS Bedrock** and send applications to recruiters for all jobs on that date.\n\n"
...
bedrock_model_id = st.sidebar.text_input("Bedrock Model ID", value="amazon.titan-text-lite-v1")
...
def titan_generate(job: dict, resume_text: str, model_id: str):
    """Call Bedrock Titan to generate a custom email + ATS-optimized resume PDF."""
    prompt = f"""
...
"""
    response = run_agent_bedrock(prompt, model_id=model_id)

    import json
    try:
        result = json.loads(response)
    except Exception:
        result = {"subject": "Job Application", "email_body": response, "resume_text": resume_text}
        ...
        if jobs_df is None or jobs_df.empty:
            ...
            if matches is None:
                ...
                for i, (idx, row) in enumerate(matches.iterrows(), 1):
                    ...
                    subj, body, pdf_bytes = titan_generate(row, resume_text, model_id=bedrock_model_id)
